...
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-warning text-dark fw-semibold"><i class="fas fa-edit me-2"></i>تعديل الفئة</div>
            <div class="card-body">
                <?php if (!empty($errors)): ?>
                <div class="alert alert-danger"><?php foreach ($errors as $err): ?><div><?= htmlspecialchars($err, ENT_QUOTES, 'UTF-8') ?></div><?php endforeach; ?></div>
                <?php endif; ?>
                <form method="post" action="index.php?page=categories&action=edit&id=<?= (int)($categorie['id'] ?? 0) ?>">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                    <div class="mb-3">
                        <label class="form-label fw-semibold">اسم الفئة</label>
                        <input type="text" name="LibCat" class="form-control" value="<?= htmlspecialchars($categorie['LibCat'] ?? '', ENT_QUOTES, 'UTF-8') ?>" required autofocus>
                    </div>
